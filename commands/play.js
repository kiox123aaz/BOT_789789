...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!channel) return message.reply("**คุณต้องเข้าช่องเสียงก่อนครับ**!").catch(console.error);
    if (serverQueue && channel !== message.guild.me.voice.channel)
      return message.reply(`You must be in the same channel as ${message.client.user}`).catch(console.error);

    if (!args.length)
      return message
        .reply(`วิธีใช้คำสั่ง: ${message.client.prefix}play <ลิ้งเพลง | ชื่อเพลง>`)
        .catch(console.error);
    ...
    if (serverQueue) {
      ...
      return serverQueue.textChannel
        .send(`✅ **${song.title}** ถูกเพิ่มลงในคิวเเล้ว โดย ${message.author}`)
        .catch(console.error);
    }
